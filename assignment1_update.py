...
class KalmanFilterModel(KalmanFilterBase):
    ...
    def initialise(self, time_step):
        ...
        # Set the R Matrix
        meas_std = 10.0
        self.R = np.array([[meas_std*meas_std,0],
                          [0,meas_std*meas_std]])
    ...
    def update_step(self, measurement):
        ...
        if self.state is not None and self.covariance is not None:
            ...
            z_hat = np.matmul(self.H,x)

            # Innovation: y = z - z_hat
            y = z - z_hat

            # Innovation Covariance: S = H * P * H' + R
            S = np.matmul(self.H, np.matmul(P, np.transpose(self.H))) + self.R

            # Kalman Gain: K = P * H' * S^-1
            K = np.matmul(P, np.matmul(np.transpose(self.H), np.linalg.inv(S)))
            ...
            # I = np.eye(P.shape[0])
            P_update = np.matmul(np.eye(4) - np.matmul(K,self.H), P)
